...
function addMemberSpaceship(member: string, spaceship: { name: string, crewLimit: number, crew: string[]}) {
    if (spaceship.crew.length >= spaceship.crewLimit) {
        alert(`${member} não pode ser adicionado. População máxima excedida`)
    } else {
        spaceship.crew.push(member)
        alert(`${member} foi adicionado com sucesso a tripulação da ${spaceship.name}`)
    }
}
...
function secondMenuOption() {
    ...
    if (spaceship) {
        ...
        if (confirmation) {
            addMemberSpaceship(member, spaceship)
        }
    }
}
...
function thirdMenuOption() {
    ...
    if(spaceship) {
        const confirmation = confirm(`Confirma o envioo da ${spaceship.name} na missão?`)
        if (confirmation) {
